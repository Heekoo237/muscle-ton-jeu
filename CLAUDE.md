...
| 9 | Toute requête qui lit une **collection** doit **paginer** (`selectAll`) OU **prouver** qu'elle reste sous le plafond « Max Rows » de PostgREST (1000 par défaut). Un `.limit(N)` avec N > plafond est un **FAUX garde-fou** : le serveur plafonne la réponse quand même, sans erreur. Vérifié par `supabasePage.test.ts` |
...
**Les migrations s'appliquent À LA MAIN dans Supabase** (pas de migration automatique en CI). **Convention : quand un code dépend d'une migration, l'annoncer en PREMIÈRE LIGNE de la réponse**, l'appliquer avant déploiement, puis confirmer sur `/api/health/schema`. **Cette annonce en première ligne NE se remplace PAS par un contrôle automatique — les deux, jamais l'un à la place de l'autre.** On ne veut pas dépendre de la discipline d'ouvrir un endpoint après chaque push ; le bandeau reste. Le contrôle vient EN PLUS : `/api/health/schema` renvoie le **commit déployé** (`VERCEL_GIT_COMMIT_SHA`, comme `/version`) À CÔTÉ de ce qui manque — un objet absent se lit alors « le déploiement `<commit>` est EN AVANCE sur la base », en UN appel (plus besoin de corréler `/version` et le schéma à la main). Ça attrape en particulier les **migrations non-cassantes** (colonne pas encore lue, RLS) qui ne lèvent aucun 500 et passeraient donc inaperçues sans témoin dédié. Un **troisième** consommateur rejoint l'endpoint et `health.py` : l'action CI **`verifier-schema`** (`verify_schema.py`, lecture seule, échoue si décalage) — elle a remplacé l'ancienne `migrer-base`, dont la liste de migrations FIGÉE à 0011 « réussissait » en étant quinze migrations en retard : *une étape qui migre à moitié ment.* La surveillance couvre aussi la **RLS** des 8 tables de référence (section `rls` du manifeste, migration 0016) : une table de référence sans RLS est lisible/écrivable par la clé publique, et **aucun contrôle « objet présent » ne le verrait**. Le manifeste étant **curé** (les objets critiques, pas tous), la règle n°8 reste en partie une discipline : `tests/test_schema_manifest.py` en automatise ce qui est net (cohérence des entrées, et « toute fonction créée est au manifeste » — allowlist d'une entrée), et laisse colonnes/tables au jugement humain (« est-ce lu au point de faire un 500 ? » — un scan SQL ne le sait pas).

**Pourquoi la règle n°9 — une requête qui RÉUSSIT en étant INCOMPLÈTE.** Troisième défaut du même type, après (1) l'étape `migrer-base` figée à 0011 qui « réussissait » quinze migrations en retard, et (2) le commentaire « email d'alerte » qui n'envoyait rien. Ici : PostgREST plafonne toute réponse à « Max Rows » (1000 par défaut) **sans lever d'erreur**, et un `.limit(50000)` est plafonné pareil. La table `teams` dépassait 1000 lignes : une équipe (« Real Racing Club de Santander ») tombait hors des 1000 renvoyées → son match ressortait `non_resolu`, nom d'équipe **vide**, et l'appariement par paire scorait 0,00 sur un fixture pourtant présent. **Le code était bon ; la donnée n'arrivait jamais jusqu'à lui.** Le point commun des trois : *rien ne signale l'incomplétude* — il faut un garde-fou qui la rende bruyante. D'où `selectAll` (pagination par `.range()`, indépendante du réglage) et `supabasePage.test.ts` qui **échoue** si une lecture de collection n'est ni paginée, ni bornée par preuve (`.single`/`.maybeSingle`/`count head`/`.limit ≤ 1000`), ni inscrite à l'allowlist avec sa preuve. L'allowlist EST l'audit : chaque lecture non paginée y porte sa raison de rester sous le plafond. **Un `.limit(20000)` n'est pas une borne — c'est un vœu.**

**Terrain (2026-08) — 0016 était restée non-appliquée des mois, invisible.** Ses 8 tables de référence (`leagues`, `teams`, `fixtures`, `team_strength`, `market_map`, `correction_queue`, `league_catalog`, `vision_stats`) étaient lisibles ET écrivables avec la clé publique (les GRANT Supabase par défaut s'appliquent sans barrière quand la RLS est OFF). Deux causes : (1) le **mauvais témoin** — compter les policies (`pg_policies`) ne prouve rien (`0 policy` = deny-all si RLS active, grand ouvert sinon) ; le seul fiable est `pg_tables.rowsecurity` / `relrowsecurity`. (2) La RLS est **l'unique artefact qu'aucun chemin légitime n'exerce** (pipeline en `postgres`, app en `service_role` : tous deux BYPASSRLS) — elle ne peut donc pas planter un job pour se signaler, contrairement à une contrainte `ON CONFLICT` ou une fonction manquante. C'est exactement pourquoi le contrôle RLS la lit directement, sans attendre qu'un attaquant l'exerce.
