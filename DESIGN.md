...
| **Serré / gardé** | `--c-line-strong` | `#C9B79A` | Ligne GARDÉE mais juste au-dessus de la barre (« pas retiré » ≠ « solide »). Filet gauche 3 px `--c-line-strong` (gris **NEUTRE**) + badge `serré` en `--c-ink-3` sur `--c-canvas-sunk`, pilule 1 px. Note « On la garde, mais elle est juste au-dessus de notre barre. » | **Jamais l'ocre** (réservé au fragile — une ligne gardée n'est pas une alerte). **Jamais le barré** (réservé au retiré). Jamais d'icône triangle. C'est un 4ᵉ état discret, pas un cri. |
